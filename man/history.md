...
To view the history, use `--out-history` or `-oh`.

You can operate with history facts using commands like
`--select-fact=<fact-id>`.

The history can be deleted as a file or with the command `--remove-history` or
`-rh` or `-rmh`. In the latter case, the [memory](memory.md) connected to the
chat will also be deleted.
